...
\section{Trace, Norm and Discriminant} % Trace, Norm and Discriminant
\begin{definition}
    ...
    \begin{remark}
        Especially, $L / K$ is a finite extension of fields of degree $n$.
        In this case, let $\chi_a$ be a characteristic polynomial of $T_a$ of degree $n$, and $m_a$ be the minimal polynomial of $a$ over $K$ of degree $d=\left[K(a),K\right]$.
    \end{remark}
\end{definition}


\begin{proposition}
    Let $L / K$ be an extension of fields of degree $n$ and $a \in L$. Then
    \begin{equation*}
        \chi_a=m_a^e
    \end{equation*}
...
\begin{corollary}
    ...
    \begin{enumerate}
        ...
        \item
              ...
              $\operatorname{Norm}_{L / K}(a)=\prod_{\sigma} \sigma(a)$.
    \end{enumerate}
    \begin{proof}
...
\begin{definition}
    ...
    Let $\alpha_1, \ldots, \alpha_m$ be elements of $B$.
    We define their \textbf{discriminant} to be
    \begin{equation*}
        \disc_{B/A}\left(\alpha_1, \ldots, \alpha_m\right)=\operatorname{det}\left(\operatorname{Tr}_{B / A}\left(\alpha_i \alpha_j\right)\right)_{1 \leq i, j \leq m} .
    \end{equation*}
    ...
    \begin{remark}
        Especially, if $L / K$ is a finite separable field extension of degree $n$.
    \end{remark}
\end{definition}
...
\begin{proposition}
    ...
    Then elements $\gamma_1, \ldots, \gamma_m$ form a basis for $B$ as an $A$-module if and only if
    \begin{equation*}
        \left(\disc\left(\gamma_1, \ldots, \gamma_m\right)\right)=(\disc(B / A)) \quad(\text { as ideals in } A) .
    \end{equation*}
\end{proposition}
...
\begin{corollary}
    Let $A$ be an integral domain, $K$ its field of fractions, $L / K$ a separable finite field extension of degree $n$ and $B$ the integral closure of $A$ in $L$.
    If $B$ is free of rank $n$ over $A$, then $\operatorname{disc}(B / A) \neq 0$.
    \begin{proof}
        If $\left\{\beta_1, \ldots, \beta_m\right\}$ is a basis for $B$ as an $A$-module, then it follows easily from  that it is also a basis for $L$ as a $K$-vector space. Hence $\operatorname{disc}(B / A)$ represents $\operatorname{disc}(L / K)$.
    \end{proof}
\end{corollary}
...
\begin{proposition}
    ...
    Let $A$ be an integral domain with fraction field $K=\operatorname{Frac}(A)$ and $L/K$ be finite field extension.
    Let $B:=A_L$ be the integral closure of $A$ in $L$. Then the following statements hold:
    \begin{enumerate}
        \item
...
\begin{proposition}
    Let $A$ be an integrally closed integral domain with field of fractions $K$, and let $B$ the integral closure of $A$ in a separable extension $L$ of $K$ of degree $n$.
    \begin{enumerate}
        \item
        ...
        \item
              $B$ is free of rank $n$ over $A$ if $A$ is a principal ideal domain.

    \end{enumerate}
    \begin{remark}
        Indeed, $B$ is a finitely generated $A$-module
    \end{remark}
    \begin{proof}
...
\begin{definition}
    When $K$ is a number field ($A=\mathbb{Z}, L=\mathbb{Q}, B=\mathcal{O}_K$), the basis $\alpha_1, \ldots, \alpha_n$ for $\mathcal{O}_K$ as a free $\mathbb{Z}$-module (also a $\mathbb{Q}$-basis of $K$) is called an \textbf{integral basis}.
    \begin{remark}

        (a)
        Let $C=\sum A \beta_i \subset B$, with $\beta_i$ a basis for $L$ over $K$. Define

        \begin{equation*}
            C^*=\{\beta \in L \mid \operatorname{Tr}(\beta \gamma) \in A \text { for all } \gamma \in C\} .
        \end{equation*}
        ...
        \begin{equation*}
            \beta \in C^* \Longleftrightarrow \operatorname{Tr}\left(\beta \beta_i\right) \in A \text { for } i=1, \ldots, m,
        \end{equation*}

        and it follows that

        \begin{equation*}
            C^*=\sum A \beta_i^{\prime} .
        \end{equation*}
        ...
        \begin{equation*}
            C=\sum A \beta_i \subset B \subset \sum A \beta_i^{\prime}=C^* .
        \end{equation*}

        (b) Write $L=\mathbb{Q}[\beta]$ with $\beta \in B$, and let $f(X)$ be the minimal polynomial of $\beta$. Let $C=\mathbb{Z}[\beta]=\mathbb{Z} 1+\mathbb{Z} \beta+\cdots+\mathbb{Z} \beta^{m-1}$. We want to find $C^*$.

        One can show that
        \begin{equation*}
            \operatorname{Tr}\left(\beta^i / f^{\prime}(\beta)\right)=\left\{\begin{array}{ccc}
                0 & \text { if } & 0 \leq i \leq m-2 \\
                1 & \text { if } & i=m-1
            \end{array}\right.
        \end{equation*}
        ...
        \begin{equation*}
            \operatorname{det}\left(\operatorname{Tr}\left(\beta^i \cdot \beta^j / f^{\prime}(\beta)\right)=(-1)^m\right.
        \end{equation*}

        (the only term contributing to the determinant is the product of the elements on the other diagonal). If $\beta_0^{\prime}, \ldots, \beta_{m-1}^{\prime}$ is the dual basis to $1, \beta, \ldots, \beta^{m-1}$, so that $\operatorname{Tr}\left(\beta^i \cdot \beta_j^{\prime}\right)=\delta_{i j}$, then

        \begin{equation*}
            \operatorname{det}\left(\operatorname{Tr}\left(\beta^i \cdot \beta_j^{\prime}\right)\right)=1 .
        \end{equation*}
        ...
        \begin{equation*}
            \left\{1 / f^{\prime}(\beta), \ldots, \beta^{m-1} / f^{\prime}(\beta)\right\}
        \end{equation*}

        to the basis

        \begin{equation*}
            \left\{\beta_0^{\prime}, \ldots, \beta_{m-1}^{\prime}\right\}
        \end{equation*}

        has determinant $\pm 1$, and so it is invertible in $M_n(A)$. Thus we see that $C^*$ is a free $A$-module with basis $\left\{1 / f^{\prime}(\beta), \ldots, \beta^{m-1} / f^{\prime}(\beta)\right\}$ :

        \begin{equation*}
            C=A[\beta] \subset B \subset f^{\prime}(\beta)^{-1} A[\beta]=C^* .
        \end{equation*}
    \end{remark}
\end{definition}
...
\section{Rings of integers in Number Fields} % Rings of integers in Number Fields

\begin{definition}
    Let $K$ be a number field. A subring $\mathcal{O}$ of $\mathbb{Z}_K$ is called an \textbf{order} of $K$ if $\mathcal{O}$ has an integral basis of length $[K: \mathbb{Q}]$.
\end{definition}

\begin{corollary}
    Any order in a number field $K$ is a dedekind domain.
\end{corollary}
...
\begin{definition}
    Let $K$ be a number field with ring of integers $\mathbb{Z}_K$ and $0 \neq \mathfrak{a} \subset K$ be a finitely generated $\mathbb{Z}_K$-module.
    The \textbf{discriminant} of $\mathfrak{a}$ is defined as $\operatorname{disc}\left(\alpha_1, \ldots, \alpha_n\right)$ for any $\mathbb{Z}$-basis of the free $\mathbb{Z}$-module $\mathfrak{a}$ (see Proposition 3.14). (By Proposition 2.8 (c), this definition does not depend on the choice of $\mathbb{Z}$-basis because the basis transformation matrix is invertible with integral entries and thus has determinant $\pm 1$.)

    The discriminant of $K$ is defined as $\operatorname{disc}\left(\mathbb{Z}_K\right)$.
\end{definition}
...
\chapter{The Finiteness of the Class Number}
\section{}
