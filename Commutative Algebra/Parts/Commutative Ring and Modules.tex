...
\begin{definition}
    ...
    A \textbf{graded $A$-module} is an $A$-module $M$ together with a family $\left\{M_n\right\}_{n \geqslant 0}$ of subgroups of $M$ such that
    \begin{enumerate}[label=(\roman*)]
        \item
...
\begin{definition}
    ...
    \begin{remark}
        ...
        In many situations one further assumes that $A_0 = k$.
        In this case, $A_0$ coincides with the base ring, and $A$
        may equivalently be viewed as a graded $A_0$-algebra.
    \end{remark}
\end{definition}
...
\begin{proposition}
    ...
    \end{enumerate}
    \begin{remark}
        Noted that the associated graded ring $\operatorname{gr}_{\mathcal{F}}A$ depends on the filtration $\mathcal{F}$ and not isomorphic to $A$ in general.
    \end{remark}
\end{proposition}
...
\begin{definition}% Hilbert Function
    Let $R=\bigoplus_{n\geq 0} R_n$ be a commutative Noetherian graded ring, $M=\bigoplus_{n\geq 0} M_n$ a finitely generated graded $R$-module and assume that each $M_n$ is a finitely generated $R_0$-module with a additive function $\lambda$.
    \begin{enumerate}
        \item
...
\begin{corollary}
    There exists a $\chi(t,R,M)\in \mathbb{Q}[t]$ of degree $\dim M$ such that
    \begin{equation*}
