...
\begin{definition}
    ...
    In particular,  $\left(0: \mathfrak{b}\right)$ is the \textbf{annihilator} of $\mathfrak{b}$.

    In this notation the set of all zero-divisors in $R$ is
    \begin{equation*}
...
\begin{proposition}
    ...
    (4) $\left(\bigcap_i \mathfrak{a}_i: \mathfrak{b}\right)=\bigcap_i\left(\mathfrak{a}_i: \mathfrak{b}\right)$

    (5) $\left(\mathfrak{a}: \sum_i \mathfrak{b}_i\right)=\bigcap_i\left(\mathfrak{a}: \mathfrak{b}_i\right)$.
\end{proposition}
...
\begin{corollary}
    If $D$ is a division ring, then the matrices ring $M_{n} \left(D\right)$ of all $n \times n$ matrices over $D$ is both Artinian and Noetherian.
    \begin{proof}
        It suffices to show that $R=M_{n} \left(D\right)$ has a composition series of left ideal and a composition of right ideals.
        Let left ideals
        \begin{equation*}
            R_i=Re_1+Re_2+\ldots+Re_i
        \end{equation*}
        and right ideals
        \begin{equation*}
            R_i^\prime=e_1R+e_2R+\ldots+e_iR
        \end{equation*}
        Verify that each $R_i$ is a left ideal of $R$ and that $R_i / R_{i-1} \cong R e_i$ which has no nonempty proper submodules , whence $R=R_n \supset R_{n-1} \supset \cdots \supset R_1 \supset R_0=0$ is a composition series of left ideals.
    \end{proof}
\end{corollary}
...
\chapter{The Hilbert Nullstellensatz} % The Hilbert Nullstellensatz



\begin{definition}
    Let $k$ be a field and $F$ is an algebraically closed extension field of $K$.

    If $S$ is a subset of $K\left[x_1, \ldots, x_n\right]$, the set of all zeros of $S$ is called the \textbf{affine $K$-variety} (or algebraic set) in $F^n$ defined by $S$ and is denoted $V(S)$. Thus
    \begin{equation*}
        V(S)=\left\{\left(a_1, \ldots, a_n\right) \in F^n \mid f\left(a_1, \ldots, a_n\right)=0 \text { for all } f \in S\right\} .
    \end{equation*}
    Note that if $I$ is the ideal of $K\left[x_1, \ldots, x_n\right]$ generated by $S$, then $V(I)=V(S)$.

    The assignment $S \mapsto V(S)$ defines a function from the set of all subsets of $K\left[x_1, \ldots, x_n\right]$ to the set of all subsets of $F^n$. Conversely, define a function from the set of subsets of $F^n$ to the set of subsets of $K\left[x_1, \ldots, x_n\right]$ by $Y \mapsto J(Y)$, where $Y \subset F^n$ and
    \begin{equation*}
        J(Y)=\left\{f \in K\left[x_1, \ldots, x_n\right] \mid f\left(a_1, \ldots, a_n\right)=0 \text { for all }\left(a_1, \ldots, a_n\right) \in Y\right\} .
    \end{equation*}
    Note that $J(Y)$ is actually an ideal of $K\left[x_1, \ldots, x_n\right]$.
\end{definition}

\begin{theorem}
    [Hilbert Nullstellensatz]
    Let $F$ be an algebraically closed extension field of a field $K$ and $I$ a proper ideal of $K\left[x_1, \ldots, x_{n}\right]$.Then
    \begin{equation*}
        \operatorname{Rad} \left(I\right)
        =
        J(V(I))
    \end{equation*}
    In other words, $f\left(a_1, \ldots, a_{n}\right)=0$ for every zero $\left(a_1, \ldots, a_{n}\right)$ of I in $F^{n}$ if and only if $f^{m} \in I$ for some $m \geq 1$.
\end{theorem}
...
\begin{definition}
    Let $R$ be a commutative ring with identity and $M$ a $R$-module.
    \begin{enumerate}
        \item An ideal $\mathfrak{q}$ in $R$ is \textbf{primary} if $\mathfrak{q} \neq R$ and if
              \begin{equation*}
    ...
    \end{enumerate}
    \begin{remark}
        If we view $R$ as itself $R$-module, the two definition are equivalent for $R$.
    \end{remark}
\end{definition}
...
\begin{theorem}
    ...
    \begin{enumerate}
        \item
              If $\mathfrak{q}$ is a primary ideal in $R$, ideal $\mathfrak{p}=\Rad(\mathfrak{q})$ is a prime ideal containing $\mathfrak{q}$.
        ...
        \item
              If $N$ is a primary submodule of $M$, $\left(N:M\right)=
                  \left\{r\in R \mid rM\subset N\right\}$
              is a primary ideal in $R$.
              Thus $\mathfrak{p}
                  =
                  \Rad\left(N:M\right)
                  =
                  \left\{r \in R \mid r^n M \subset N \text{ for some } n>0\right\}$ is a prime ideal in $R$ .
...
\begin{theorem}
    ...
    Let $R$ be a commutative ring, $\mathfrak{q}$ and $\mathfrak{p}$ be ideals in $R$.
...
\begin{corollary}
    ...
    Let $R$ be a commutative ring with identity, if $\mathfrak{q}_1, \mathfrak{q}_2, \ldots, \mathfrak{q}_{n}$ are $\mathfrak{p}$-primary, then $\bigcap_{i=1}^n \mathfrak{q}_i$ is also $\mathfrak{p}$-primary.
    \begin{proof}
...
\begin{lemma}[Nakayama]
    \label{lem: Nakayama lemma}
    If $J$ is an ideal in a commutative ring $R$ with identity, then the following conditions are equivalent.
    \begin{enumerate}
        \item
              $J$ is contained in every maximal ideal of $R$ ;
        \item
              $1_{R}-j$ is a unit for every $j \in J$
        \item
              If $M$ is a finitely generated $R$-module such that $JM=M$, then $M=0$;
        \item
              If $N$ is a submodule of a finitely generated $R$-module $M$ such that $M=JM+N$, then $M=N$.
        \item
              for every Noetherian $R$-module $M$, $\bigcap_{n=1} J^n M=0$.
    \end{enumerate}
    \begin{proof}
        It is easy to verify that 1. $\Leftrightarrow$ 2. $\Leftrightarrow$ 3. $\Leftrightarrow$ 4.

        ($\Rightarrow$ 5.)
        If $N=\bigcap_n J^n M$, then $J N=N$ by \ref{thm: Krull Intersection Theorem}.
        Since $N$ is finitely generated by \ref{thm: equivalent conditions of Noetherian modules}, $B=0$ by 3.

        ($\Leftarrow$ 5.)
        We may assume $R \neq 0$. If $M$ is any maximal ideal of $R$, then $M \neq R$ and $A=R / M$ is a nonzero $R$-module that has no proper submodules by \ref{thm: The Fourth or Lattice Isomorphism Theorem of Modules}.
        Thus $A$ trivially satisfies the ascending chain condition, whence $\bigcap J^n A=0$ by hypothesis. Since $J A$ is a submodule of $A$, either $J A=A$ or $J A=0$. If $J A=A$, then $J^n A=A$ for all $n$. Consequently, $\bigcap_n J^n A=A \neq 0$, which is a contradiction. Hence $J A=0$. But $0=J A=J(R / M)$ implies that $J \subset J R \subset M$.
    \end{proof}
\end{lemma}
