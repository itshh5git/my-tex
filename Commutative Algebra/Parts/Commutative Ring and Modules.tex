...
\begin{definition}
    ...
    \begin{enumerate}
        \item
              A left $A$-module $M$ is said to satisfy the \textbf{ascending chain condition (ACC) on submodules} (or to be \textbf{Noetherian}) if for every chain $M_1 \subset M_2 \subset M_3 \subset \cdots$ of submodules of $M$, there is an integer n such that $M_{i}=M_{n}$ for all $i \geq n$.

        \item
              The ring $A$ is \textbf{left Noetherian} if left regular module ${}_AA$ is Noetherian.
              $A$ is said to be \textbf{Noetherian} if $A$ is both left and right Noetherian.

        \item
              A left $A$-module $N$ is said to satisfy, the \textbf{descending chain condition (DCC) on submodules} (or to be \textbf{Artinian}) if for every chain $N_1 \supset N_2 \supset N_3 \supset \cdots$ of submodules of $N$, there is an integer $m$ such that $N_{i}=N_m$ for all $i \geq m$.

        \item
              $A$ is \textbf{left Artinian } if ${}_A A$ is Artinian.

              $A$ is said to be \textbf{Artinian} if $A$ is both left and right Artinian.
    \end{enumerate}
\end{definition}

\begin{definition}
    Let $A$ be a ring, a left $A$-module $M$ is said to satisfy the \textbf{maximum condition} [resp. minimum condition] on submodules if every nonempty set of submodules of $M$ contains a maximal [resp. minimal] element (with respect to set theoretic inclusion).
\end{definition}
...
\begin{theorem}
    ...
    A module satisfies the ascending [resp. descending] chain condition on submodules if and only if $A$ satisfies the maximal [resp. minimal] condition on submodules.
\end{theorem}

\begin{theorem}
    Let $A$ be a ring and $0 \rightarrow M^\prime \xrightarrow{} M \xrightarrow{} M^{\prime\prime} \rightarrow 0$ be a short exact sequence of left $A$-modules.
    Then ${}_A M$ is Noetherian [resp. Artinian] if and only if ${}_A M^\prime$ and ${}_A M^{\prime\prime}$ are both Noetherian [resp. Artinian].
\end{theorem}
...
\begin{corollary}
    ...
    \begin{enumerate}
        \item
              If $M_1$ is a submodule of a module $M$ , then $M$ is Noetherian [resp. Artinian] if and only if $M_1$ and $M / M_1$ satisfiy it.

        \item
              If $M_1, \ldots, M_n$ are modules, then the direct sum $M_1 \oplus M_2 \oplus \cdots \oplus M_n$ is Noetherian [resp. Artinian] if and only if each $M_i$ is Noetherian [resp. Artinian].
        \item
              If $A$ is a left Noetherian [resp. Artinian] ring, then every finitely generated left $A$-module $M$ is Noetherian [resp. Artinian].
        \item
              the homomorphism image of a Noetherian [resp. Artinian] module is Noetherian [resp. Artinian].
    \end{enumerate}
\end{corollary}
...
\begin{definition}
    Let $A$ be a ring and a left $A$-module $M$.
    \begin{enumerate}
        \item
              A \textbf{normal series} for $M$ is a chain of submodules: $M=M_0 \supset M_1 \supset$ $M_2 \supset \cdots \supset M_n$. The factors of the series are the quotient modules
              \begin{equation*}
                  M_i / M_{i+1} \quad(i=0,1, \ldots, n-1) .
              \end{equation*}
              The \textbf{length} of the normal series is the number of proper inclusions ( $=$ number of nontrivial factors).

        \item
              A \textbf{refinement} of the normal series $M=M_0 \supset M_1 \supset \cdots \supset M_n$ is a normal series obtained by inserting a finite number of additional submodules between the given ones. A \textbf{proper refinement} is one which has length larger than the original series.
...
\begin{theorem}
    ...
    Let $A$ be a ring.
    A left $A$-module $M$ has a composition series if and only if $M$ satisfies both the ACC and DCC on submodules.
    \begin{proof}
...
\section{Hilbert's Basis Theorem} % Hilbert's Basis Theorem
\begin{theorem}
    Let $A$ be a left Noetherian ring, then $A[x_1,\ldots,x_n]$ is also left Noetherian.
    \begin{proof}
        We prove the case $n=1$; the general case follows by induction on $n$.

        Let $\mathfrak{a}$ be an left ideal of $A[x]$.
        For each nonnegative integer $d$, let $\mathfrak{a}_d$ be the set of all leading coefficients of polynomials in $\mathfrak{a}$ of degree $d$ together with 0.
        Then $\mathfrak{a}_d$ is a left ideal of $A$ for each $d$ and $\mathfrak{a}_0 \subset \mathfrak{a}_1 \subset \mathfrak{a}_2 \subset \cdots$ is an ascending chain of left ideals of $A$.
        Since $A$ is left Noetherian, there exists an integer $n$ such that $\mathfrak{a}_d=\mathfrak{a}_n$ for all $d \geq n$.

        For each $d=0,1, \ldots, n$, let $a_{d 1}, a_{d 2}, \ldots, a_{d m_d}$ be generators of the left ideal $\mathfrak{a}_d$.
        For each $a_{d i}$, choose a polynomial $f_{d i}(x) \in \mathfrak{a}$ of degree $d$ with leading coefficient $a_{d i}$.

        We claim that the finite set of polynomials
        \begin{equation*}
            \left\{f_{d i}(x) \mid d=0,1, \ldots, n ; i=1,2, \ldots, m_d\right\}
        \end{equation*}
        generates the ideal $\mathfrak{a}$.

        Let $f(x)$ be any polynomial in $\mathfrak{a}$.
        If $\deg f(x)=d$, then the leading coefficient of $f(x)$ is in $\mathfrak{a}_d=\mathfrak{a}_n$, so there exist elements $b_i \in A$ such that
        \begin{equation*}
            \text { leading coefficient of } f(x)=\sum_{i=1}^{m_n} b_i a_{n i}
        \end{equation*}
        Let $g(x)=\sum_{i=1}^{m_n} b_i x^{d-n} f_{n i}(x)$.
        Then $g(x) \in \mathfrak{a}$ and $\deg g(x)=d$ with leading coefficient equal to that of $f(x)$. Thus $\deg\left(f(x)-g(x)\right)<d$ and by repeating this process, we can express $f(x)$ as an $A[x]$-linear combination of the polynomials $f_{d i}(x)$. Therefore, $\mathfrak{a}$ is finitely generated and $A[x]$ is left Noetherian.
    \end{proof}
\end{theorem}

\begin{corollary}
    The finitely generated algebra over a left Noetherian ring is left  Noetherian.
\end{corollary}

\begin{proposition}
    Let $A \subseteq B \subseteq C$ be rings. Suppose that $A$ is Noetherian, that $C$ is finitely generated as an $A$-algebra and that $C$
    is either (i) finitely generated as a $B$-module or (ii) integral over $B$.
    Then $B$ is finitely generated as an A-algebra.
\end{proposition}


\section{Commutative Noetherian Rings}




\begin{proposition}
    Let $A$ be a subring of $B$; suppose that $A$ is Noetherian and that $B$ is finitely generated as an $A$-module. Then $B$ is a Noetherian ring.
\end{proposition}
...
\begin{theorem}[Hilbert's Basis Theorem]
    If $A$ is a commutative Noetherian ring with identity, then so is $R\left[x_1, \ldots, x_{n}\right]$ and $R\left[\left[x\right]\right]$
\end{theorem}
...
\begin{lemma}
    If $A$ is a left Artinian ring, then $J(A)$ is nilpotent.
    \begin{proof}
        Consider the descending chain of ideals
        \begin{equation*}
            J \supset J^2 \supset J^3 \supset \cdots
        \end{equation*}
        there exists an integer $n$ such that $J^n=J^{n+1}=\cdots$.
        Denote $J^n$ by $I$, then $I=J  I$.
        Assume $I \neq 0$ and let nonempty set $I^2=I\neq 0$
        \begin{equation*}
            \mathcal{S}
            :=
            \left\{\text{left ideal } L:IL\neq 0\right\}
        \end{equation*}
        Then there exists a minimal element $L_0$ in $\mathcal{S}$ by \ref{thm: chain condition and m condition}.
        Furthermore, $L_0=Aa$ for some $a \in L_0$ and $IL_0=L_0$ by minimality.
        Hence, there exists a nonzero $x \in I\subset J^n$ such that $x a=a$, that is, $1_A - x$ is a left zero divisor which contradicts the definition of $J$.
        Therefore, $J^n=I=0$.
    \end{proof}
\end{lemma}

\begin{lemma}
    Let $A$ is simple left Artinian ring, then the following condition are equivalent for a left $A$-module $M$:
    \begin{enumerate}
        \item Noetherian
        \item Artinian
        \item has finite length.
    \end{enumerate}
\end{lemma}
...
\begin{theorem}[Hopkins-Levitzki]
    ...
    Let $A$ be a left Artinian ring and a left $A$-modules $M$.
    If $M$ is Artinian, then $M$ is Noetherian.
    \begin{proof}
        Consider the descending chain of submodules
        \begin{equation*}
            M \supset J M \supset J^2 M \supset \cdots \supset J^n M =0
        \end{equation*}
        where $J=J(A)$.
        Then $J^kM$ and $J^k M / J^{k+1} M$ are Artinian for all $k$ by \ref{cor: chain condition on submodules of direct sum }.

        On the other hand, $J^k M / J^{k+1} M$ is a module over the semisimple Artinian ring $A / J$, and is also Artinian. Thus each $J^k M / J^{k+1} M$ is Noetherian and thus $M$ is Noetherian by \ref{cor: chain condition on submodules of direct sum }.
    \end{proof}
\end{theorem}

\begin{example}
    Let ring $A=\left(\begin{matrix}
                \mathbb{Q} & \mathbb{R} \\
                0          & \mathbb{R}
            \end{matrix}\right)$, $A$ is right Artinian but not left Artinian.
    ...
    \end{equation*}
    where $\left\{V_n\right\}$ is a infinite descending chain of $\mathbb{Q}$-subspace of $\mathbb{R}$.
    ...
    \begin{equation*}
        ...
        \end{matrix}\right)
    \end{equation*}
    where $D_1,D_2$ are rings and $M$ is a $\left(D_1,D_2\right)$-bimodule.
    Then $T$ is left Artinian if and only if $D_1,D_2$ are left Artinian and $M_{D_2}$ has finite length.

\end{example}


\section{Commutative Artinian Rings} % Commutative Artinian Rings
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item
              $\Spec \left(A\right)$ is finite and Krull $\dim A=0$

        \item
              (Structure theorem) $A$ is isomorphic to a finite direct product of Artin local rings.
    \end{enumerate}
    \begin{proof}
        1. $\Rightarrow$ 2.
        By \ref{thm:Hopkins-Levitzki}, $A$ is Noetherian. If $\mathfrak{p}$ is a prime ideal in $A$, then $A / \mathfrak{p}$ is an integral domain and Artinian, whence a field. Thus $\dim A=0$.

        2. $\Rightarrow$ 3.
        Let $\mathcal{S}:=\left\{\mathfrak{p}_1\cap \mathfrak{p}_1\cdots \cap \mathfrak{p}_r:\mathfrak{p}_i \text{ maximal ideal}\right\}$, then $\mathcal{S}$ has a minimal element $\mathfrak{a}=\mathfrak{p}_1\cap \mathfrak{p}_1\cdots \cap \mathfrak{p}_n$ by \ref{thm: chain condition and m condition}.
        Hence for any maximal ideal $\mathfrak{p}$, $\mathfrak{p}\supset \mathfrak{a}$ by minimality of $\mathfrak{a}$, whence $\mathfrak{m}=\mathfrak{m}_i$ for some $i$ by .
    \end{proof}
\end{theorem}

\begin{corollary}
    In a commutative Artin ring the
    \begin{enumerate}
        \item
              nilradical is equal to the Jacobson radical.
        \item
              $A$ has finitely many maximal ideals
    \end{enumerate}
\end{corollary}
...
\chapter{The Structure of Rings and Modules} % The Structure of Rings and Modules
\minitoc

\section{Simplicity} % Simplicity
\begin{definition}
    A ring $A$ is said to be \textbf{simple} if $A$ has no proper two-sided ideals.
\end{definition}

\begin{definition}
    A left module $M$ over a ring $A$ is said to be \textbf{simple} (or \textbf{irreducible}) if $M$ has no proper submodules.
    \begin{remark}
        A left ideal $\mathfrak{a}$ of $A$ is a simple left $A$-module if and only if $\mathfrak{a}$ is a minimal left ideal of $A$.
        In this case, we call $\mathfrak{a}$ the \textbf{simple left ideal} of $A$.
    \end{remark}
\end{definition}
\begin{proposition}
    \label{pro: Proposition of simple module}
    Let $A$ be a ring and $M$ be a simple left $A$-module, then
    \begin{enumerate}
        \item
              $M=Am$ for every $0 \neq m \in M$.
        \item
              If $0 \neq u \in M$, then $M \cong A / \Ann(u)$, thus $\Ann\left( u \right)$ is a left maximal ideal.

              Conversely, if $\mathfrak{m}$ is left maximal in $A$, then $A/\mathfrak{m}$ is a simple $A$-module with $\Ann\left( A/\mathfrak{m} \right)=\mathfrak{m}$
        \item
              If $A$ is not a division ring, then $M$ is a torsion module.$m\in M$.
    \end{enumerate}
\end{proposition}

\begin{lemma}[Schur]
    \label{lem: Schur's lemma}
    Let $M$ be a simple left module over a ring $A$ and let $N_i$ be any $A$-module.
    \begin{enumerate}
        \item
              Every nonzero $A$-module homomorphism $f: M \rightarrow N_1$ is a monomorphism;
        \item
              Every nonzero $A$-module homomorphism $g: N_2 \rightarrow M$ is an epimorphism;

        \item
              The endomorphism ring $D=\operatorname{Hom}_{A}(M,M)$ is a division ring,
              then $M$ is a vector space over $\operatorname{Hom}_A(M,M)$ with $f a=f(a)$
    \end{enumerate}
\end{lemma}
...
\begin{definition}
    ...
    \begin{enumerate}
        \item
              The ring $A$ is said to be \textbf{left primitive} if there exists a simple faithful left $A$-module.
        \item
              An two-sided ideal $\mathfrak{a}$ of a ring $A$ is said to be \textbf{left primitive} if the quotient ring $A / \mathfrak{a}$ is a left primitive ring.
    \end{enumerate}
    \begin{remark}
        If $M$ is a simple left $A$-module, then $A/\Ann\left( M \right)$ is left primitive with faithful simple left $A/\Ann\left( M \right)$-module $M$.
    \end{remark}
\end{definition}
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item
              A simple ring $A$ is left primitive with faithful simple left $A$-module $A/\mathfrak{m}$ for any left maximal ideal $\mathfrak{m}$ of $A$.

        \item
              A commutative ring $A$ is primitive if and only if $A$ is a field.
    \end{enumerate}
\end{proposition}
...
\begin{definition}
    ...
    A subring $A$ of $\End_D(V)$ is called a \textbf{dense ring of endomorphisms} of $V$ if for every positive integer $n$, every linearly independent subset $\left\{u_1, \ldots, u_{n}\right\}$ of $V$ and every arbitrary subset $\left\{v_1, \ldots, v_{n}\right\}$ of $V$ , there exists $f \in A$ such that $f\left(u_{i}\right)=v_{i}, (i=1,2, \ldots, n)$.
\end{definition}
...
\begin{lemma}
    ...
    Let $M$ be a simple module over a ring $A$.
    Consider $M$ as a vector space over the division ring $D=\End_A(M)$.
    If $V$ is a finite dimensional $D$-subspace of $M$ and $a \in M-V$, then there exists $r \in A$ such that $ra \neq 0$ and $rV=0$.
    \begin{remark}
        In other words, the element $r\in \Ann_A\left( V \right)$ only annihilates $D$-subspace $V$.
    \end{remark}
    \begin{proof}
...
\begin{theorem}[Classic Jacobson Density Theorem]
    ...
    Let $A$ be a left primitive ring with a faithful simple left $A$-module $M$ and division ring $D=\End_{A}(M)$.
    Then $A$ is a dense ring of endomorphisms of the $D$-vector space $M$ (viewed $\alpha :A \hookrightarrow \End_D(M)$ by $ a\mapsto \alpha_a$ where left regular action $\alpha_a: m\mapsto am$ in $M$).
    \begin{proof}
        It clear that $\alpha: A \rightarrow \End_D(M)$ is a ring monomorphism since $M$ is faithful.
...
\begin{corollary}
    If $M$ is a simple left $A$-module, then $A$ acts densely on $M$.
    That is, for every positive integer $n$, every linearly independent subset $\left\{u_1, \ldots, u_n\right\}$ of $M$ and every arbitrary subset $\left\{v_1, \ldots, v_n\right\}$ of $M$, there exists $a \in A$ such that $a u_i=v_i, (i=1,2, \ldots, n)$.
\end{corollary}



\subsection{Artinian simple ring}% Artinian simple ring
\begin{lemma}
    \label{lem: Dense ring of endomorphisms of a finite dimensional vector space}
    Let $A$ be a dense ring of endomorphisms of a vector space $V$ over a division ring $D$.
    Then $A$ is left Artinian if and only if $\operatorname{dim}_{D} V$ is finite, in which case $A=\End_D(V) \cong M_n\left(D\right)$.

    \begin{proof}
        If $A$ is Artinian and $\operatorname{dim}_D V$ is infinite, then there exists an infinite linearly independent subset $\left\{u_1, u_2, \ldots\right\}$ of $V$.
        The $V$ is a left simple $\End_D(V)$-module and hence a left simple $A$-module.

        For each $n$, let $I_n=\Ann_{A}\left\{u_1, \ldots, u_n\right\}$.
        Then $I_1\supsetneq I_2\supsetneq\cdots$ is a properly descending chain by \cref{lem: Lemma of Jacobson Density Theorem}, which is a contradiction. Hence $\dim_D V$ is finite.

        Conversely if $\operatorname{dim}_D V$ is finite, then $V$ has a finite basis $\left\{v_1, \ldots, v_m\right\}$. Then $A= \operatorname{Hom}_D(V, V)\cong M_n\left(D\right) $ is Artinian.
    \end{proof}
\end{lemma}


\begin{corollary}[Artin-Wedderburn]
    If $A$ is a simple ring, then
    \begin{enumerate}
        \item
              $A$ is left Artinian
        \item
              $A\cong M_n\left(D\right)$ for some positive integer $n$ and some division ring $D\cong \End_A(A/\mathfrak{m})$ for some maximal left ideal $\mathfrak{m}$ of $A$.
        \item
              $A$ is right Artinian
    \end{enumerate}
\end{corollary}
...
\begin{lemma}[Schur's lemma]
    \label{lem: Schur's lemma for endomorphism ring}
    ...
    \begin{remark}
        That is, that is,
        \begin{equation*}
            g\in \End_{\End_D(V)}(V)\cong D
        \end{equation*}
        since $V$ is a simple $\End_D(V)$-module.
    \end{remark}
\end{lemma}

\begin{lemma}
    \label{lem:simple module over matrix ring}
    The simple left $M_n(D)$-modules for some division ring $D$ is isomorphic to $D^n$ with the usual left matrix multiplication.
\end{lemma}

\begin{proposition}
    If $M_{n_1}(D_1)\cong M_{n_2}(D_2)$ for some positive integers $n_1,n_2$ and some division rings $D_1,D_2$, then $n_1=n_2$ and $D_1\cong D_2$.
    \begin{proof}
        \begin{equation*}
            D_1
            \cong
            \End_{M_{n_1}(D_1)}\left( D_1^{n_1} \right)
            \cong
            \End_{M_{n_2}(D_2)}\left( D_2^{n_2} \right)
            \cong
            D_2
        \end{equation*}
    \end{proof}
...
\begin{theorem}
    ...
    Let $A$ be a ring and $M$ a left $A$-module.
    ...
    \begin{remark}
        A left $A$-module $M$ satisfying the three conditions is said to be \textbf{semisimple}.
...
\begin{definition}
    A ring $A$ is called \textbf{semisimple} if $1_A\neq 0$, and if ${}_A A$ is a semisimple module.
\end{definition}
...
\begin{proposition}
    If $A$ is a semisimple ring, then $A$ is left Artinian.
    \begin{proof}
        Since ${}_A A $ is semisimple module, there exist simple left ideals $L_j$ of $A$ such that
        \begin{equation*}
            {}_A A
            \cong
            \bigoplus_{j \in J} L_j
        \end{equation*}
        with projection maps $\pi_j:{}_A A \rightarrow {}_A L_j$ for each $j \in J$.
        Since the sum is direct, $\left\{j_k: \pi_{j_k}(1_A) \neq 0\right\}$ is finite.
        Then
        $J=\left\{j_k: \pi_{j_k}(1_A) \neq 0\right\}$ is finite and
        \begin{equation*}
            {}_A A
            \cong
    ...
    \end{proof}
    \begin{remark}
        It is clear that any simple left ideal $L$ of $A$ is isomorphic to some $L_j$, thus $A$ has finitely many minimal left ideals up to isomorphism.
    \end{remark}
\end{proposition}
...
\begin{theorem}[Wedderburn-Artin]
    If $A$ is a semisimple ring, then we have ring isomorphisms
    \begin{equation*}
        A
        \cong
        \prod_{i=1}^t  A_i
    ...
    \end{equation*}
    where $A_i\cong M_{n_i}\left(D_i\right)$ $i \in I$.
    \begin{proof}
        Let $\left\{L_{j_i}\right\}_{i=1}^t$ be all distinct simple left ideals of $A$ up to isomorphism and define $A_i$ be the sum
        \begin{equation*}
            {}_A A_i
            :=
        ...
        \]
        Then $A_i$ is a two-sided ideal of $A$ by \cref{lem:lemma of Wedderburn} and hence a ring with identity element $e_i=\pi_i(1_A)$ ($\pi_i$ the projection from $A$ to $A_i$).
        The projection from $A$ to $A_i$ induce a ring homomorphism
        \begin{equation*}
            A\cong \prod_{i=1}^t A_i
        \end{equation*}
        since $1_A=e_1+e_2+\cdots+e_t$ and $e_i e_j=\delta_{ij} 1_A$.

        Finally, each $A_i$ is a simple ring since any nonzero two-sided ideal $\mathfrak{a}$ of $A_i$ contains a minimal left ideal $L$ of $A_i$ and hence a minimal left ideal of $A$ isomorphic to $L_i$. Then $Lb\subset L$ runs over all minimal left ideals of $A$ isomorphic to $L_i$ by \cref{lem:lemma of Wedderburn}, whence $A_i \subset L\subset \mathfrak{a}$.
    \end{proof}
\end{theorem}

\subsection{Structure of modules over semisimple rings} % Structure of modules over semisimple rings
...
\begin{theorem}
    ...
    \begin{equation*}
        A\cong \prod_{i=1}^t A_i
    \end{equation*}
    where each $A_i\cong M_{n_i}(D_i)$.
    Then every left $A$-module $M$ is isomorphic to
    \begin{equation*}
        \bigoplus_{i=1}^t e_iM
        \cong
        \bigoplus_{i=1}^t S_i^{\left(\kappa_i\right)}
    \end{equation*}
    where $e_iM\cong S_i\cong D_i^{n_i}$  and $\kappa_i$ is a cardinal number.
...
\begin{definition}% Hilbert Function
    Let $A=\bigoplus_{n\geq 0} A_n$ be a commutative Noetherian graded ring and $M=\bigoplus_{n\geq 0} M_n$ a finitely generated graded $A$-module (It follows that each $M_n$ is a finitely generated $A_0$-module with a additive function $\lambda$).
    \begin{enumerate}
        \item
    ...
    \end{enumerate}
    \begin{remark}
        If $A_0$ is Artinian ring, $\lambda(N)=\ell_{A_0}(N)$. If $A_0$ is a field, $\lambda(N)=\dim_{A_0}(N)$.
    \end{remark}
\end{definition}
...
\begin{theorem}[Hilbert-Serre]
    ...
    \begin{equation*}
        \frac{f(t)}{\prod_{i=1}^d\left(1-t^{k_i}\right)}
    \end{equation*}
    where $f(t)\in \mathbb{Z}[t]$ and $k_1, \ldots, k_d$ are the degrees of the homogeneous generators of $A$ over $A_0$.
    The order of the pole of $P_M^\lambda(t)$ at $t=1$ is called the \textbf{dimension} of $M$ with respect to $\lambda$, denoted by $\dim M$.
    \begin{proof}
...
\begin{corollary}
    There exists a polynomial $\chi(t,A,M)\in \mathbb{Q}[t]$ such that
    \begin{equation*}
