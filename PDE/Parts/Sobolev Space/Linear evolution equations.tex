...
\begin{definition}
    \begin{equation*}
        L^2\left(0, T ; H_0^1(U)\right)
        :=
        \left\{\mathbf{u}:[0, T] \rightarrow H_0^1(U) \mid \int_0^T\|\mathbf{u}(t)\|_{H_0^1(U)}^2 \d t<\infty\right\}
    \end{equation*}
    with norm $\left\|\mathbf{u}\right\|_{L^2(0,T;H_0^1(U))}=\left(\int_0^T\|\mathbf{u}(t)\|_{H_0^1(U)}^2 \d t\right)^{1/2}$

    The derivatives of $\mathbf{u}$ are defined in the distributional sense. 
\end{definition}
...
\begin{definition}
    We will first study the initial Dirichlet problem
    \begin{equation*}
        \left\{\begin{aligned}
            u_t+\mathcal{L} u=f & \text { in } U_T                     \\
    ...
    The letter $\mathcal{L}$ denotes for each time $t$ a second-order partial differential operator, having either the divergence form
    \begin{equation*}
        \mathcal{L} u=-\sum_{i, j=1}^n\left(a^{i j}(x, t) u_{x_i}\right) x_j+\sum_{i=1}^n b^i(x, t) u_{x_i}+c(x, t) u
    \end{equation*}
    or else the nondivergence form
    \begin{equation*}
        \mathcal{L} u=-\sum_{i, j=1}^n a^{i j}(x, t) u_{x_i x_j}+\sum_{i=1}^n b^i(x, t) u_{x_i}+c(x, t) u
    \end{equation*}
