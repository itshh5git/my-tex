...
\subsection{Topological Properties of Manifolds} % Topological Properties of Manifolds
\begin{proposition}
...
\begin{definition}
    Let $M$ be a smooth $n$-manifold, $k$ is a nonnegative integer, and $f: M \rightarrow \mathbb{R}^k$ is any function. We say that $f$ is \textbf{smooth at $p$} if there exists a chart $(U, \varphi)$ for $M$ whose domain contains $p$ and such that the composite function
    \begin{equation*}
        f \circ \varphi^{-1} : \varphi(U)\rightarrow
        \mathbb{R}^k
    \end{equation*}
    is smooth at $\varphi(p)$ (i.e. smooth on a neighborhood of $\varphi(p)$ in $\varphi(U)$).
...
\subsection{Partial Derivatives and the Rank of a Smooth Map} % Partial Derivatives and the Rank of a Smooth Map

On a manifold $M$ of dimension $m$, let $(U, \phi)$ be a chart.
As a function into $\mathbb{R}^m$, $\phi$ has $m$ components $x^1, \ldots, x^m$ defined by
\begin{equation*}
    \phi(p)=\left(x^1(p), \ldots, x^m(p)\right)
\end{equation*}
This means that if $r^1, \ldots, r^m$ are the standard coordinates on $\mathbb{R}^m$, then $x^i=r^i \circ \phi$.


\begin{definition}
    Let $M$ be a manifold of dimension $m$, and let $(U, \varphi)=\left(U,x^1,\ldots,x^m\right)$ be a chart on $M$.For  $f\in C^\infty(U,\mathbb{R})$ and a point $p\in M$, we define the \textbf{partial derivative of $f$ with respect to $x^i$ at $p$} to be
    \begin{equation*}
        \frac{\partial (f \circ \varphi^{-1})}{\partial r^i}(\varphi(p))
    \end{equation*}
    \begin{remark}
        Usually written as $\left. \frac{\partial}{\partial x^i}\right|_p f$ or $  \frac{\partial f}{\partial x^i}(p)$. Thus
        \begin{equation*}
            \left.\frac{\partial}{\partial x^i}\right|_p f
            =
            \frac{\partial f}{\partial x^i}(p)
            =
            \frac{\partial\left(f \circ \phi^{-1}\right)}{\partial r^i}(\phi(p))
            =
            \left.\frac{\partial}{\partial r^i}\right|_{\phi(p)}\left(f \circ \phi^{-1}\right) .
        \end{equation*}
        on $U$,
        and
        \begin{equation*}
            \frac{\partial f}{\partial x^i} \circ \phi^{-1}=\frac{\partial\left(f \circ \phi^{-1}\right)}{\partial r^i} .
        \end{equation*}
        on $\phi(U)$.
    \end{remark}
\end{definition}
...
\begin{definition}
    Let $f: N \rightarrow M$ be a smooth map, and let $(U, \varphi)=\left(U,x^1,\ldots,x^n\right)$ and $(V, \psi)=\left(V,y^1,\ldots,y^m\right)$ be charts on $M$ and $N$ respectively such that $f(U) \subset V$. Denote by
    \begin{equation*}
        f^i:=y^i \circ f : U \rightarrow \mathbb{R}
    \end{equation*}
    Then the matrix
    \begin{equation*}
        \left[\frac{\partial f^i}{\partial x^j} \right]_{\varphi(p)}
    \end{equation*}
    is called the \textbf{Jacobian matrix of $f$ relative to the charts $(U, \varphi)$ and $(V, \psi)$}. It is also written as $\partial\left(f^1,\ldots,f^n\right)/\partial\left(x^1,\ldots,x^m\right)$.
    We define the rank of $f$ at $p$
    \begin{equation*}
        \rank_p f := \rank \left(\frac{\partial f^i}{\partial x^j} \right)_{\varphi(p)}
    \end{equation*}
    \begin{remark}
        The rank of $f$ at $p$ is independent of the choice of charts.
    \end{remark}
\end{definition}
...
\begin{definition}
    Let $M$ be a $C^\infty$ manifold, $p$ be a point in $M$.
    \begin{enumerate}
        \item
              ...
              \end{equation*}
        \item
              A \textbf{tangent vector} (or say \textbf{derivation}) at $p$ is  a derivation of $\mathbb{R}$-algebra $ C^\infty_p\left(M\right)$
...
\begin{definition}
    Let $F:N \rightarrow M$ be smooth map between manifolds and a point $p\in N$.
    We define a map induced by $F$ on the tangent spaces
    \begin{equation*}
        d F_p: T_p N \rightarrow T_{F(p)} M
    \end{equation*}
    as follows.
    Given $v \in T_p N$, we let $d F_p(v)$ be the derivation at $F(p)$ that acts on $C^{\infty}_{F(p)}(M)$ by the rule
    \begin{equation*}
        \left\langle d F_p(v),  \varphi \right\rangle
        =
        \left\langle  v,  \varphi \circ F \right\rangle \quad \text{ for all } \varphi \in C_{F(p)}^\infty(M)
    \end{equation*}
\end{definition}
...
\begin{definition}
    Let $F: N \rightarrow M$ be a smooth map between smooth manifolds with or without boundary, and let $p \in N$.
    \begin{enumerate}
        \item
              We define the rank of $\boldsymbol{F}$ at $\boldsymbol{p}$ to be the rank of the linear map $d F_p: T_p N \rightarrow T_{F(p)} M$.
        ...
        \item
              $f$ is said \textbf{submersion} at $p$ if $\d f_p$ is surjective.

              $f$ is said \textbf{inmersion} if $\d f_p$ is injective
    \end{enumerate}
\end{definition}


\section{Constant Rank Theorem} % Constant Rank Theorem
\begin{theorem}[Constant rank theorem]
    Let $N$ and $M$ be manifolds of dimensions $n$ and $m$ respectively. Suppose $f: N \rightarrow M$ has constant rank $k$ in a neighborhood of
    a point $p$ in $N$. Then there are charts $\left(U, \phi\right)$ centered at $p$ in $N$ and $\left(V, \psi\right)$ centered at $f(p)$ in $M$ such that for $\left(r^1, \ldots, r^n\right)$ in $\phi(U)$,
...
\begin{theorem}[Constant-rank level set theorem]
    ...
    \begin{proof}
        Let $p$ be an arbitrary point in $f^{-1}(c)$. By the constant rank theorem there are a coordinate chart $(U, \phi)=\left(U, x^1, \ldots, x^n\right)$ centered at $p \in N$ and a coordinate chart $(V, \psi)=\left(V, y^1, \ldots, y^m\right)$ centered at $f(p)=c \in M$ such that

        \begin{equation*}
            \left(\psi \circ f \circ \phi^{-1}\right)\left(r^1, \ldots, r^n\right)=\left(r^1, \ldots, r^k, 0, \ldots, 0\right) \in \mathbb{R}^m .
        \end{equation*}


        This shows that the level set $\left(\psi \circ f \circ \phi^{-1}\right)^{-1}(0)$ is defined by the vanishing of the coordinates $r^1, \ldots, r^k$.
        The image of the level set $f^{-1}(c)$ under $\phi$ is the level set $\left(\psi \circ f \circ \phi^{-1}\right)^{-1}(0)$ (Figure 11.1), since
        \begin{equation*}
            \phi\left(f^{-1}(c)\right)=\phi\left(f^{-1}\left(\psi^{-1}(0)\right)=\left(\psi \circ f \circ \phi^{-1}\right)^{-1}(0) .\right.
        \end{equation*}


        Thus, the level set $f^{-1}(c)$ in $U$ is defined by the vanishing of the coordinate functions $x^1, \ldots, x^k$, where $x^i=r^i \circ \phi$. This proves that $f^{-1}(c)$ is a regular submanifold of $N$ of codimension $k$.
    \end{proof}
\end{theorem}
