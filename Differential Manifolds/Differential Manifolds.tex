...
And view the $T^k(V^*)\cong T^k(V)^*:=\Hom_{\mathbb{R}}\left(V^{\otimes k} ,\mathbb{R} \right)$ as the space of multilinear maps from $V^k$ to $\mathbb{R}$ (with the convention that $T^0(V^*)=\mathbb{R}$).


\begin{definition}
    \begin{enumerate}
        \item The \textbf{tensor algebra} of $V^*$ is the direct sum of all tensor powers of $V^*$:
              \begin{equation*}
                  T(V^*)
                  :=
                  \bigoplus_{k=0}^\infty T^k(V^*)
              \end{equation*}
              with the
    ...
    \begin{enumerate}
        \item associative
              \begin{equation*}
                  ...
                  \omega \wedge(\eta \wedge \xi)
                  =
                  \frac{(k+l+m)!}{k!l!m!} \operatorname{Alt}(\omega \otimes \eta \otimes \xi)
              \end{equation*}
        \item bilinear
...
\begin{corollary}
    Given a basis $\left\{f^1, \ldots, f^n\right\}$ for $V^*$, the set $\left\{f^{i_1}\wedge \cdots \wedge f^{i_k}  | 1 \leq i_1 < \cdots < i_k \leq n\right\}$ is a basis for $\Lambda^k(V^*)$ .
\end{corollary}
    ...
    \begin{enumerate}
        \item
              ...
              \begin{equation*}
                  \Lambda(V^*)
                  :=
                  \bigoplus_{k=0}^\infty \Lambda^k(V^*)
              \end{equation*}
              with the wedge product as multiplication.

        \item
              The \textbf{symmetric algebra} is the graded algebra
              \begin{equation*}
                  \operatorname{Sym}(V^*)
                  :=
                  \bigoplus_{k=0}^\infty \operatorname{Sym}^k(V^*)
              \end{equation*}
...
\begin{definition}
    The tnesor space of type $(k,l)$ at $p$ is the vector space
    \begin{equation*}
        T^{(k,l)}_p M
        :=
        T^k(T_p M) \otimes T^l\left(T_p^* M\right)
    \end{equation*}
    The \textbf{tensor bundle} of type $(k,l)$ over $M$ is the vector bundle
    \begin{equation*}
        T^{(k,l)}M
        :=
        \coprod_{p\in M} T^{(k,l)}_p M
        =
    \end{equation*}
    The \textbf{tensor fields} of type $(k,l)$ on $M$ are the smooth sections of $T^{(k,l)}M\to M$.
    \begin{remark}
        Given a smooth chart $\left(U,x^i\right)$, we can write the $(k,l)$-tensor fields $A$ (omit the base point) in coordinates locally as
        \begin{equation*}
            A
            =
            A^{i_1\ldots i_k}_{j_1\ldots j_l} \cdot \frac{\partial}{\partial x^{i_1}}\otimes \cdots \otimes \frac{\partial}{\partial x^{i_k}} \otimes dx^{j_1} \otimes \cdots \otimes dx^{j_l}
        \end{equation*}
    \end{remark}
\end{definition}


\subsubsection{Pullback and Lie Derivative of Tensor Fields}
...
\begin{proposition}
    Interior multiplication is an antiderivation of degree $-1$ on the exterior algebra:
    \[
...
\begin{definition}
    ...
    \begin{enumerate}
        \item
              ...
              \]
        \item
              A \textbf{differential $k$-form} on $M$ is a smooth section of $\Lambda^k(T^*M) \to M$. The space of all smooth $k$-forms is denoted by
              \[
    ...
    \begin{remark}
        ...
        \]
        where the coefficient functions $\omega_{i_1\cdots i_k}$ are smooth on $U$ and multiindex $I=(i_1,\ldots,i_k)$.
    \end{remark}
\end{definition}
...
\section{Inner Products, Lie Derivatives and Exterior Derivatives}
...
\end{definition}
\begin{definition}
    The \textbf{exterior derivative} is the unique map
    \begin{equation*}
...
\begin{definition}
    A form $\omega$ is called \textbf{closed} if $d\omega=0$. It is called \textbf{exact} if there exists a form $\eta$ such that $\omega=d\eta$
    \begin{remark}
...
\begin{theorem}[Cartan's Magic Formula]
    On a smooth manifold, for every smooth vector field $X$ and every differential form $\omega$,
    \[
        \mathcal{L}_X\omega
        =
        \iota_X(d\omega)+d(\iota_X\omega).
    \]
\end{theorem}
    ...
    \begin{enumerate}
        ...
        \item
              ...
              \[
                  f^*(d\omega)=d(f^*\omega)
              \]
        \item The Lie derivative commutes with pullback:
    \end{enumerate}
...
\begin{theorem}[Local Slice Criterion for Embedded Submanifolds]
    If $S\subset M$ is an embedded $k$-dimensional submanifold, then $S$ satisﬁes the local $k$-slice condition.
\end{theorem}
\begin{theorem}
