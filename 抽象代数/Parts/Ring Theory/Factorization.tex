\chapter{Factorization in Integral Domaina} % Factorization in Integral Domaina
\minitoc

\section{Divisor Decomposition} % Divisor Decomposition
\subsection{Basic definition} % Basic definition
\begin{definition}
    Let $R$ be a integral domain and let $a, b \in R$ with $b \neq 0$.
    \begin{enumerate}
        \item
              $a$ is said to be a \textbf{multiple} of $b$ if there exists an element $x \in R$ with $a=b x$. In this case $b$ is said to divide $a$ or be a \textbf{divisor} of $a$, written $b \mid a$.
...
\begin{theorem}
    Let $R$ be a integral domain, and $a, b,u$ be elements of $R$.
    \begin{enumerate}
        \item
              ${a} \mid {b}$ if and only if $({b}) \subset({a})$.

        \item
              $a$ and $b$ are associates $\Leftrightarrow (a)=(b) \Leftrightarrow a=bu$ for some unit $u$.
    \end{enumerate}
\end{theorem}
...
\begin{definition}
    ...
    \end{enumerate}
    \begin{remark}
        Every associate of an irreducible [resp. prime] element is irreducible [resp. prime].
    \end{remark}
\end{definition}
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item  $c$ is irreducible if and only if
              $(c)$ is maximal in the set $\mathcal{S}$ of all proper principal ideals of $R$.
...
\end{theorem}
\begin{definition}
    Let $X$ be a nonempty subset of a integral domain $R$
    \begin{enumerate}
        \item
    ...
    \end{enumerate}
    \begin{remark}
        The greatest common divisor and least common multiple are unique up to association.
    \end{remark}
\end{definition}

\begin{definition}
    Let $R$ be a integral domain, and $a_1, a_2, \ldots, a_n$ have $1_R$ as a greatest common divisor, then $a_1, a_2, \ldots a_n$ are said to be \textbf{relatively prime}.
\end{definition}
\begin{theorem}
...
\section{E.D $\Rightarrow$ P.I.D $\Rightarrow$ U.F.D}
...
\begin{definition}
    A \textbf{principal ideal ring} is a ring in which every ideal is principal.
\end{definition}
...
\begin{theorem}
    ...
    \begin{enumerate}[resume]
        \item
              $R$ is a unique factorization domain thus
              \cref{pro: unique factorization domain} holds.
        \item
              maximal ideals and prime ideals coincide.
              \item  if $\left(a\right)+\left(b\right)=\left(c\right)$, then $c$ is a greatest common divisor of $a$ and $b$.
    \end{enumerate}
\end{theorem}
...
\begin{theorem}
    ...
    \begin{proof}
        Suppose that $R$ has a Dedekind-Hasse norm $N$. 
        Let $\mathfrak{a}$ be a nonzero ideal in $R$ and let $b$ be a nonzero element of $\mathfrak{a}$ of minimum norm. If $a \in \mathfrak{a}$, then either $a \in (b)$ or there exist $s, t \in R$ with $0<N(s a-t b)<N(b)$. The latter is impossible since $s a-t b \in \mathfrak{a}$ and $b$ has minimum norm in $\mathfrak{a}$. Therefore, $a \in (b)$ and consequently, $\mathfrak{a}=(b)$. Thus $R$ is a principal ideal domain.
...
\begin{theorem}[The division algorithm]
    Let $R$ be a ring and $f, g \in R[x]$ nonzero polynomials such that the leading coefficient of $g$ is a unit in $R$.
...
\begin{definition}
    ...
    \begin{equation*}
        ...
        \sum_{i=0}^n a_i x^i
    \end{equation*}
    a nonzero polynomial in $D[x]$. A greatest common divisor of the coefficients $a_0, a_1, \ldots, a_n$ is called a \textbf{content} of $f$ and is denoted $\operatorname{Cont}(f)$.

    If $f \in D[x]$ and $\operatorname{Cont}(f)$ is a unit in $D$, then $f$ is said to be \textbf{primitive}. Clearly for any polynomial $g \in D[x], g=\operatorname{Cont}(g) g_1$ with $g_1$ primitive.
\end{definition}


\begin{theorem}[Gauss]
    \label{thm: Gauss's lemma}
    ...
    \begin{proof}
        $f=\operatorname{Cont}(f) f_1$ and $g=\operatorname{Cont}(g) g_1$ with $f_1, g_1$ primitive. Consequently, $\operatorname{Cont}(g)=C\left(\operatorname{Cont}(f) f_1 \operatorname{Cont}(g) g_1\right) \approx \operatorname{Cont}(f) \operatorname{Cont}(g) C\left(f_1 g_1\right)$. Hence it suffices to prove that $f_1 g_1$ is primitive (that is, $C\left(f_1 g_1\right)$ is a unit).
