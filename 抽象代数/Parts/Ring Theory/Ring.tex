\chapter{Ring Theory}

\section{Basic Definition} % Basic Definition
\begin{definition}
    A \textbf{ring} $A$ is a set together with two binary operations $+$ and $\times$ satisfying the following axioms:
    \begin{enumerate}[label=(\roman*)]
        \item
              $(A,+)$ is an abelian group,
        \item
              $\times $ is associative

        \item
              the distributive laws hold in $A$ : for all $a, b, c \in A$
              \begin{equation*}
                  (a+b) \times c=(a \times c)+(b \times c) \quad \text { and } \quad a \times(b+c)=(a \times b)+(a \times c)
              \end{equation*}
    \end{enumerate}
    The ring $A$ is \textbf{commutative} if
    \begin{enumerate}[resume,label=(\roman*)]
        \item
              multiplication $\times$ is commutative.
    \end{enumerate}
    The ring $A$ is called unital if it has an \textbf{identity} $1_A$  s.t.
    \begin{enumerate}[resume,label=(\roman*)]
        \item
              $1_A \times a=a \times 1_A=a$ for all $a \in A$.
    \end{enumerate}
    \begin{remark}
        In this book, we usually use the term "ring" to refer a unital ring.
    \end{remark}
\end{definition}

\begin{definition}
    A \textbf{subring} of $A$ is a subgroup of $A$ that is closed under multiplication and contains the identity element $1_A$.
\end{definition}



\begin{proposition}
    Let $A$ be a ring. Then
    \begin{enumerate}
        \item
              $0 a=a 0=0$ for all $a \in R$.
        \item  $(-a) b=a(-b)=-(a b)$ for all $a, b \in R$.
        \item
              $(-a)(-b)=a b$ for all $a, b \in R$.
        \item
              the identity is unique and $-a=(-1) a$.
    \end{enumerate}
\end{proposition}


\begin{definition}
    Let $A$ be a ring.
    \begin{enumerate}
        \item
              A nonzero element $a$ of $A$ is called a \textbf{zero divisor} if there is a nonzero element $b$ in $A$ such that either $a b=0$ or $b a=0$.

        \item
              An element $u$ of $A$ is called a \textbf{unit} in $A$ if there is some $v$ in $A$ such that $u v=v u=1$. The set of units in $A$ is denoted $A^{\times}$.
    \end{enumerate}
\end{definition}


\begin{definition} %% integral domain
    A commutative ring with identity $1 \neq 0$ is called an \textbf{integral domain} if it has no zero divisors.
\end{definition}

\begin{proposition}[Cancellation property] %% Cancellation property
    Let $R$ be a ring. Then
    \begin{enumerate}
        \item
              Assume $a, b$ and $c$ are elements of $R$ with $a$ not a zero divisor. If $a b=a c$, then either $a=0$ or $b=c$.

        \item In particular, if $a, b, c$ are any elements in an integral domain and $a b=a c$, then either $a=0$ or $b=c$.

        \item Any finite integral domain is a field.
    \end{enumerate}
\end{proposition}

\begin{definition}
    Let $A$ be a ring.
    If there is a least positive integer $n$ such that $nr=0$ for all $r \in A$, then $A$ is said to have \textbf{characteristic} $n$ . If no such $n$ exists $A$ is said to have characteristic zero. (Notation: $\operatorname{char}{A}={n}$).
\end{definition}


\begin{theorem}
    Let $A$ be a ring with identity $1_{A}$ and characteristic ${n}>0$.
    \begin{enumerate}
        \item
              If $\varphi: \mathbb{Z} \rightarrow A$ is the map given by ${m} \mapsto m 1_{{A}}$, then $\varphi$ is a homomorphism of rings with kernel $\langle{n}\rangle=n\mathbb{Z}$.

        \item
              $n$ is the least positive integer such that ${n} 1_{{A}}=0$.

        \item
              If $A$ has no zero divisors (in particular if $A$ is an integral domain), then $n$ is prime.
    \end{enumerate}
\end{theorem}


\section{Ideal} % Ideal
\subsection{Definition and Quotient Ring} % Definition and Quotient Ring
\begin{definition}
    Let $A$ be a ring.
    A subset $\mathfrak{a}$ of $A$ is a \textbf{left ideal} of $A$ if
    \begin{enumerate}[label=(\roman*)]
        \item
              $\mathfrak{a}$ is an additive subgroup of $A$,
        \item
              $\mathfrak{a}$ is closed under left multiplication by elements from $A$, i.e., $r \mathfrak{a} \subseteq \mathfrak{a}$ for all $r \in A$.
    \end{enumerate}
    A subset $\mathfrak{b}$ that is both a left ideal and a right ideal is called an \textbf{ideal} (or a \textbf{two-sided ideal}) of $A$.
    An ideal $\mathfrak{b}$ is \textbf{proper} if $\mathfrak{b} \neq A$ and $\mathfrak{b} \neq 0$. The ideal $\{0\}$ is called the \textbf{trivial ideal} and is denoted by $0$.
\end{definition}

\begin{theorem}
    Let $A$ be a ring.
    A nonempty subset $\mathfrak{a}$ of a ring $A$ is a left [resp. right] ideal if and only if for all $a,{b} \in {\mathfrak{a}}$ and ${r} \in A$ :
    \begin{enumerate}[label=(\roman*)]
        \item
              $a, {b} \in {\mathfrak{a}} \quad \Rightarrow \quad a-{b} \in {\mathfrak{a}}$
        \item
              $a \in {\mathfrak{a}}, {r} \in {A} \quad \Rightarrow \quad {ra} \in {\mathfrak{a}}$
              $[$resp. ${ar} \in {\mathfrak{a}}$$]$
    \end{enumerate}
\end{theorem}

\begin{corollary}
    \label{cor: ideal}
    Let $R$ be a ring.
    \begin{enumerate}
        \item If $\left\{I_\alpha \right\}$ is a family of ideals [resp. left ideal] in a ring $R$, then
              \begin{equation*}
                  \bigcap I_\alpha
              \end{equation*}
              is also a ideal [resp. left ideal].

        \item If $\left\{J_\beta\right\}$ is a chain of ideals [resp. left ideal] in $\mathcal{P}(R)$ i.e. either $J_\beta \subset$ or $J_{\beta'} \subset J_\beta$ holds for all $\beta,\beta'$, then

              \begin{equation*}
                  \bigcup J_\beta
              \end{equation*}

              is also a ideal [resp. left ideal] of $R$.
    \end{enumerate}
\end{corollary}

\begin{definition}
    Let $A$ be a ring and let $\mathfrak{a}$ be an ideal of $A$. Then the (additive) quotient group $A / \mathfrak{a}$ is a ring called the \textbf{quotient ring of $A$ by $\mathfrak{a}$} under the binary operations:
    \begin{equation*}
        (r+\mathfrak{a})+(s+\mathfrak{a})=(r+s)+\mathfrak{a} \quad \text { and } \quad(r+\mathfrak{a}) \times(s+\mathfrak{a})=(r s)+\mathfrak{a}
    \end{equation*}
    for all $r, s \in A$.
    Conversely, if $\mathfrak{b}$ is any subgroup such that the above operations are well defined, then $\mathfrak{b}$ is an ideal of $A$.
    \begin{remark}
        If $\mathfrak{a}$ is a left [resp. right] ideal of $A$, then the quotient group $A / \mathfrak{a}$ is left [resp. right] $A$-module under the operation
    \end{remark}
\end{definition}


\subsection{Ideals generated by a set}
% Ideals generated by a set
\begin{definition}
    Let $X_1, X_2, \ldots, X_n$ be nonempty subsets of a ring $A$.
    Then
    \begin{enumerate}
        \item
              Denote by $X_1+X_2+\cdots+X_n$ the set
              \begin{equation*}
                  \left\{x_1+x_2+\cdots+x_n : x_i \in X_i, i=1,2, \ldots, n\right\}
              \end{equation*}

        \item
              If $X$ and $Y$ are nonempty subsets of $A$ let $X Y$ denote the set of all finite sums
              \begin{equation*}
                  \left\{x_1 y_1+\cdots+x_n y_n : n \in \mathbb{Z}_{\geq 1} ; x_i \in X , y_i \in Y \right\}
              \end{equation*}
              If $X$ consists of a single element $x$, we write $x Y$ for $X Y$. Similarly if $Y=\{y\}$, we write $X y$ for $X Y$.
    \end{enumerate}
\end{definition}

\begin{theorem}
    Let $A, A_1, A_2, \ldots, A_{{n}}, {B}$ and $C$ be subsets of ring $A$.
    Then
    \begin{enumerate}
        \item  $(A+{B})+{C}=A+({B}+{C})$; $A+B=B+A$

        \item $(AB) {C}=A(BC)$

        \item ${B}\left(A_1+A_2\right)={BA}_1+{BA}_2$; and $\left(A_1+A_2\right) {C}=A_1 {C}+A_2 {C}$.
    \end{enumerate}
\end{theorem}

\begin{definition}
    Let $X$ be a subset of a rng $R$. Let $\left\{I_\alpha \right\}$ be the family of all (left) ideals in $R$ which contain $X$.
    Then $\bigcap I_\alpha $ is called the \textbf{(left) ideal generated by $X$}. This ideal is denoted $(X)$. The elements of $X$ are called \textbf{generators} of the ideal $(X)$.

    If $X=\left\{x_1, \ldots, x_n\right\}$, then the ideal $(X)$ is denoted by $\left(x_1, x_2, \ldots, x_n\right)$ and said to be \textbf{finitely generated}.

    An ideal $(x)$  generated by a single element is called a \textbf{principal ideal}. A principal ideal ring is a ring in which every ideal is principal. A principal ideal ring which is an integral domain is called a \textbf{principal ideal domain}.
\end{definition}

\begin{proposition}
    Let $A$ be a ring and $X \subset A$,
    then
    \begin{enumerate}
        \item
              the left [resp. right] ideal generated by $X$ is
              \begin{equation*}
                  \left(X\right)_l
                  =
                  AX
                  \quad  \text{  [resp.] }
                  \left(X\right)_r= XA
              \end{equation*}
              the (two-sided) ideal generated by $X$ is
              \begin{equation*}
                  \left(X\right)
                  =
                  AX+XA +AXA
              \end{equation*}
        \item
              if $A$ is commutative, $\left(X\right)_l=\left(X\right)_r=\left(X\right)=AX$
    \end{enumerate}
\end{proposition}





\subsection{Prime ideal} % Prime ideal
\begin{definition}
    Let $A$ be a ring.
    An ideal $\mathfrak{p}$ in $A$ is said to be \textbf{prime} if
    \begin{enumerate}[label=(\roman*)]
        \item
              $\mathfrak{p} \neq {A}$
        \item
              for any ideals $A, B$ in $A$,
              $\mathfrak{a} \mathfrak{b} \subset \mathfrak{p}\Rightarrow \mathfrak{a} \subset\mathfrak{p} \text { or } \mathfrak{b} \subset \mathfrak{p}$
    \end{enumerate}
    The set of all prime ideals in a ring $A$ is called the \textbf{spectrum} of $A$, denoted by $\operatorname{Spec}(A)$.
\end{definition}

\begin{theorem}
    \label{thm: Prime ideal}
    Let $A$ be a ring and an ideal $\mathfrak{p} \neq A$.
    \begin{enumerate}
        \item
              If $a b \in \mathfrak{p} \Rightarrow a \in \mathfrak{p}\text { or  } b \in \mathfrak{p}$ for all $a,b\in A$, then $\mathfrak{p}$ is prime.
    \end{enumerate}
    If $A$ is commutative,
    \begin{enumerate}[resume]
        \item ideal $\mathfrak{p}$ is prime if and only if then $\mathfrak{p}$ satisfies the above condition.
    \end{enumerate}
\end{theorem}

\begin{corollary}
    \label{cor: Prime ideal}
    Let $A$ be a commutative ring and $\mathfrak{p}$ be an ideal in $A$. The following conditions are equivalent.
    \begin{enumerate}
        \item
              Ideal $\mathfrak{p}$ is prime
        \item
              $A-\mathfrak{p}$ is a multiplicative set of $A$.
        \item
              ${A} / \mathfrak{p} $ is an integral domain.
    \end{enumerate}
\end{corollary}


\begin{proposition}
    \label{thm: lemma of primary decomposition}
    Let $K$ be a subring of a commutative ring $R$.
    If $\mathfrak{p}_1, \ldots, \mathfrak{p}_{\mathrm{n}}$ are prime ideals of R such that $K\subset \mathfrak{p}_1 \cup \mathfrak{p}_2 \cup \ldots \cup \mathfrak{p}_{\mathrm{n}}$, then $K \subset \mathfrak{p}_{i}$ for some $i$ .
    \begin{proof}
        Assume $K \not \subset \mathfrak{p}_i$ for every $i$. It then suffices to assume that $n>1$ and $n$ is minimal; that is, for each $i, K \not \subset \bigcup_{j \neq i} \mathfrak{p}_j$.
        For each $i$ there exists $a_i \in K-\bigcup_{j \neq i} \mathfrak{p}_1$. Since $K \subset \bigcup_i \mathfrak{p}_i$, each $a_i \in \mathfrak{p}_i$. The element $a_1+a_2 a_3 \cdots a_n$ lies in $K$
        and hence in $\bigcup_i \mathfrak{p}_i$. Therefore $a_1+a_2 a_3 \cdots a_n=b_j$ with $b_j \in \mathfrak{p}_j$. If $j>1$, then $a_1 \in \mathfrak{p}_j$, which is a contradiction. If $j=1$, then $a_2 a_3 \cdots a_n \in \mathfrak{p}_1$, whence $a_i \in \mathfrak{p}_1$ for some $i>1$ by \ref{cor: Prime ideal}.
    \end{proof}
\end{proposition}






\subsection{Maximal ideal} % Maximal ideal

\begin{definition}
    Let $A$ be a ring. An ideal $\mathfrak{m}$ is called a \textbf{maximal ideal} [resp. maximal left ideal]
    if
    \begin{enumerate}[label=(\roman*)]
        \item
              $\mathfrak{m} \neq A$
        \item
              the only ideals[resp. left ideal] containing $\mathfrak{m}$ are $A$ and $\mathfrak{m}$.
    \end{enumerate}
\end{definition}


\begin{theorem}
    In a ring $A$ with identity $1_A\neq 0$, every ideal [resp. left ideal] in $A$ (except $A$ itself) is contained in a [resp. left ideal] maximal ideal.
    \label{thm: existence of maximal ideal}
    \begin{proof}
        It follows from Zorn's lemma and \cref{cor: ideal}
    \end{proof}
\end{theorem}

\begin{theorem}
    \label{thm: maximal ideal is prime}
    If $A$ is a commutative ring, then every maximal ideal $\mathfrak{m}$ is prime.
\end{theorem}


\begin{theorem}
    \label{thm: maximal ideal}
    Let $\mathfrak{m}$ be an ideal in a ring $A$ with identity $1_A \neq 0$.
    \begin{enumerate}
        \item
              If $\mathfrak{m}$ is maximal and $A$ is commutative, then the quotient ring $A / \mathfrak{m}$ is a field.

        \item
              If the quotient ring $A / \mathfrak{m}$ is a division ring, then $\mathfrak{m}$ is maximal.
    \end{enumerate}
\end{theorem}



\subsection{Chinese Remainder Theorem}
Assume that $A$ is a commutative ring with identity.
\begin{definition}
    The ideals $\mathfrak{a}$ and $\mathfrak{b}$ of the ring $A$ are said to be \textbf{comaximal} if $\mathfrak{a}+\mathfrak{b}=A$.
\end{definition}

\begin{lemma}
    If $\mathfrak{a}_1$ and $\mathfrak{a}_2$ are comaximal ideals in $A$, then $\mathfrak{a}_1 \mathfrak{a}_2=\mathfrak{a}_1 \cap \mathfrak{a}_2$.
\end{lemma}

\begin{theorem}[Chinese Remainder Theorem]
    Let $A$ be a commutative ring with $1_A\neq 0$ and $\mathfrak{a}_1, \mathfrak{a}_2, \ldots, \mathfrak{a}_k$ be ideals in $A$. Then
    \begin{enumerate}
        \item
              The map
              \begin{equation*}
                  A \rightarrow A / \mathfrak{a}_1 \times A / \mathfrak{a}_2 \times \cdots \times A / \mathfrak{a}_k
              \end{equation*}
              defined by
              \begin{equation*}
                  r \mapsto\left(r+\mathfrak{a}_1, r+\mathfrak{a}_2, \ldots, r+\mathfrak{a}_k\right)
              \end{equation*}
              is a ring homomorphism with kernel $\mathfrak{a}_1 \cap \mathfrak{a}_2 \cap \cdots \cap \mathfrak{a}_k$.

        \item
              If $\mathfrak{a}_i$ are comaximal in pairs, then this map is surjective and
              \begin{equation*}
                  \quad A /\left(\mathfrak{a}_1 \mathfrak{a}_2 \cdots \mathfrak{a}_k\right)=A /\left(\mathfrak{a}_1 \cap \mathfrak{a}_2 \cap \cdots \cap \mathfrak{a}_k\right) \cong A / \mathfrak{a}_1 \times A / \mathfrak{a}_2 \times \cdots \times A / \mathfrak{a}_k
              \end{equation*}
    \end{enumerate}
\end{theorem}

\begin{corollary}
    Let $n$ be a positive integer and let $p_1^{\alpha_1} p_2^{\alpha_2} \ldots p_k^{\alpha_k}$ be its factorization into powers of distinct primes. Then
    \begin{equation*}
        \mathbb{Z} / n \mathbb{Z}
        \cong
        \left(\mathbb{Z} / p_1^{\alpha_1} \mathbb{Z}\right) \times\left(\mathbb{Z} / p_2^{\alpha_2} \mathbb{Z}\right) \times \cdots \times\left(\mathbb{Z} / p_k^{\alpha_k} \mathbb{Z}\right)
    \end{equation*}
    as rings, so in particular we have the following isomorphism of multiplicative groups:
    \begin{equation*}
        (\mathbb{Z} / n \mathbb{Z})^{\times} \cong\left(\mathbb{Z} / p_1^{\alpha_1} \mathbb{Z}\right)^{\times} \times\left(\mathbb{Z} / p_2^{\alpha_2} \mathbb{Z}\right)^{\times} \times \cdots \times\left(\mathbb{Z} / p_k^{\alpha_k} \mathbb{Z}\right)^{\times} .
    \end{equation*}
\end{corollary}



\section{Homomorphisms}  % Homomorphisms

\begin{definition}
    Let $R$ and $S$ be ring.
    \begin{enumerate}
        \item
              A \textbf{ring homomorphism} is a map $f: R \rightarrow S$ satisfying
              \begin{equation*}
                  f(a+b)=f(a)+f(b), \quad
                  f(a b)=f(a) f(b)
              \end{equation*}
              for all $a, b \in R$ and $f(1_R)=1_S$.
        \item
              The \textbf{kernel} of the ring homomorphism $f$, denoted $\Ker f$, is the set of elements of $R$ that map to $0$ in $S$.

        \item
              A bijective ring homomorphism is called an \textbf{isomorphism}.
    \end{enumerate}
\end{definition}


\begin{theorem}[The First Isomorphism Theorem for Rings]
    If $f: R \rightarrow S$ is a homomorphism of rings, then
    \begin{enumerate}
        \item
              The kernel of $f$ is an ideal of $R$.

        \item
              The image of $f$ is a subring of $S$ and $R / \Ker f\cong f(R)$.
    \end{enumerate}
\end{theorem}

\begin{corollary}
    If $I$ is any ideal of $R$, then the \textbf{natural projection}
    \begin{equation*}
        R \rightarrow R / I \quad \text { defined by } \quad r \mapsto r+I
    \end{equation*}
    is a surjective ring homomorphism with kernel $I$. Thus every ideal is the kernel of a ring homomorphism.
\end{corollary}


\begin{theorem}[The Second Isomorphism Theorem for Rings]
    Let $R$ be a ring. Let $A$ be a subring and let $B$ be an ideal of $R$. Then $A+B=\{a+b \mid a \in A, b \in B\}$ is a subring of $R, A \cap B$ is an ideal of $A$ and \begin{equation*}
        (A+B) / B \cong A /(A \cap B)
    \end{equation*}
\end{theorem}


\begin{theorem}[The Third Isomorphism Theorem for Rings]
    Let $I$ and $J$ be ideals of $R$ with $I \subseteq J$. Then $J / I$ is an ideal of $R / I$ and
    \begin{equation*}
        (R / I) /(J / I) \cong R / J
    \end{equation*}
\end{theorem}

\begin{theorem}[The Fourth or Lattice Isomorphism Theorem for Rings]
    Let $\mathfrak{a}$ be an ideal of ring $R$.
    \begin{enumerate}
        \item
              The correspondence
              \begin{equation}
                  R \leftrightarrow R / \mathfrak{a}
              \end{equation}
              is an inclusion preserving bijection between the collection of subrings of $R$ that contain $I$ and the collection of subrings of $R / \mathfrak{a}$.

        \item
              Furthermore, a subring $A$ containing $\mathfrak{a}$ is an  ideal of $R$ if and only if $A / \mathfrak{a}$ is an ideal of $R / \mathfrak{a}$.
    \end{enumerate}
\end{theorem}



\section{Rings of Polynomial and Formal Power Series}
We only consider $A\left[x\right]$ where $A$ is a commutative ring with identity.

\begin{proposition}
    Let $A$ be a ring with identity and $f=\sum_{i=0}^{\infty} a_{i} x^{i} \in A[[x]]$.
    \begin{enumerate}
        \item
              $f$ is a unit in $A[[x]]$ if and only if its constant term $a_0$ is a unit in $A$
        \item
              If $a_0$ is irreducible in $A$, then $f$ is irreducible in $A[[x]]$.
    \end{enumerate}
\end{proposition}


\begin{corollary}
    If $A$ is a field, then $A[[x]]$ is a local ring with unique maximal ideal $\left(x\right)]]$
\end{corollary}









