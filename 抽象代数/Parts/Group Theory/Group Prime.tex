...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item
              Define $G^{(1)}:=[G, G]$ be the \textbf{commutator subgroup of $G$}.

        \item
              The sequence $G^{(n)} := [G^{(n-1)}, G^{(n-1)}]$ (with $G^{(0)}=G$) is called the \textbf{derived series} of $G$.
        \item
              $G$ is said to be \textbf{solvable} if there is a $n>0$ such that $G^{\left(n\right)}=\left\langle e \right\rangle$ (equivalently, $G^{(n-1)}$ is abelian).
    \end{enumerate}
\end{definition}
\begin{proposition}
    ...
    \end{enumerate}
\end{proposition}
We now have the following characterization of solvable groups.
\begin{theorem}
    Let $G$ be a group of order $n$.
    Then $G$ is solvable if and only if for any decomposition $n=st$ with $\left(s,t\right)=1$ , $G$ has a subgroup of order $s$.
    \begin{proof}
        If $G$ is solvable, then $G^{(n)}=\langle e\rangle$ for some $n>0$. Let $N=G^{(n-1)}$. Then $N \unlhd G$ and $G / N$ is abelian. Since $\left(s, t\right)=1$, there are subgroups of $G / N$ of order $s$ and $t$. The inverse image of the subgroup of order $s$ under the canonical projection $G \rightarrow G / N$ is a subgroup of $G$ of order $s$.
