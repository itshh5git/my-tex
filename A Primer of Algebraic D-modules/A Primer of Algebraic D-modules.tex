...
\begin{definition}
    ...
    \begin{enumerate}
        \item
              ...
              \begin{equation*}
                  D_i(g)=\Delta^{-1} \cdot \operatorname{det} J\left(F_1, \ldots, F_{i-1}, g, F_{i+1}, \ldots, F_n\right) .
              \end{equation*}
...
\end{proposition}
The \textbf{universal cover} of $D^{\prime}(\epsilon)$ is the set $\tilde{D}(\epsilon)=\{z \in \mathbb{C}: \operatorname{Re}(z)<\log (\epsilon)\}$. The projection $\pi$ of $\tilde{D}(\epsilon)$ on $D^{\prime}(\epsilon)$ is defined by $\pi(z)=e^z$.
By lifting lemma, every holomorphic function $h$ on $D^{\prime}(\epsilon)$ can be lifted to a holomorphic function $\tilde{h}$ on $\tilde{D}(\epsilon)$ such that $h \circ \pi=\tilde{h}$.
We have the commutative diagram
\begin{equation*}
    \begin{tikzcd}
        & \tilde{D}(\epsilon) \arrow[d,"\pi"] \arrow[rd,dashed,"\exists!\tilde{h}"]& \\
        & D^{\prime}(\epsilon) \arrow[r,"h"]  & \mathbb{C}
    \end{tikzcd}
\end{equation*}
If we view $\mathcal{H}(D^\prime(\epsilon))$ as an $A_1(\mathbb{C})$-module in the natural way,
then the action of $A_1(\mathbb{C})$ on $\mathcal{H}(\tilde{D}(\epsilon))$ is given by the following formulas.
\begin{equation*}
    P \cdot h:=P\left(e^z\right) h(z) \text { and } \partial \cdot h:=h^{\prime}(z) e^{-z} .
\end{equation*}
for $P=P(x)$ and $h \in \mathcal{H}(\tilde{D}(\epsilon))$.
