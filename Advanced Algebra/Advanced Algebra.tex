...
\begin{question}
    ...
    \]
    \begin{proof}
        作多项式 $f(x)=a_1+a_2 x+a_3 x^2+\cdots+a_n x^{n-1}$ ，令 $\varepsilon_1, \varepsilon_2, \cdots, \varepsilon_n$ 是 $1$ 的所有 $n$ 次方根．又令
        \[
            {V}
            =
            \left(\begin{array}{ccccc}
...
\begin{theorem}
    ...
    \begin{enumerate}
        \item $\varphi$ 的特征子空间（如果存在的话）是 $\psi$ 的不变子空间，$\psi$ 的特征子空间是 $\varphi$ 的不变子空间．


        \item $R(\psi)$ 是$\varphi$不变子空间，$R(\varphi)$ 是$\psi$不变子空间.
    \end{enumerate}
\end{theorem}
...
\section{可对角化线性变换的判定}
\subsection{有n个线性无关的特征向量}
\subsection{有完全的特征向量系}
线性映射或矩阵可对角化的充要条件是有完全的特征向量系，即任一特征值的代数重数等于其几何重数。

\begin{question}
    Suppose $A$ is a $m\times n$ matrix, $B$ is a $n\times m$ matrix, and $\left|AB\right| \neq 0$．Then $AB$ can be diagonalized if and only if $BA$ can be diagonalized.
    \begin{proof}
...
\section{标准型的构造}
\begin{theorem}
    Given matrices $A$ and $B$ over a field $\mathbb{F}$.
    Then $A$ and $B$ are similar if and only if $\lambda I_n - A$ and $\lambda I_n - B$ have the same Smith normal form, i.e. hetong in $M_n(\mathbb{F}[\lambda])$.
\end{theorem}
