...
\begin{definition}
    Let $X$ and $Y$ be topological spaces, $f: X \rightarrow Y$ is called \textbf{Borel measurable} if it is $\left(\mathcal{B}_X, \mathcal{B}_Y\right)$-measurable.
\end{definition}

\begin{lemma}
    Let $f: X \rightarrow Y$ be a map inducing a mapping $f^{-1}: \mathcal{P}(Y) \rightarrow \mathcal{P}(X)$, defined by $f^{-1}(E)=\{x \in X: f(x) \in E\}$, which preserves arbitrary unions, intersections, and complements.
...
\begin{definition}
    Let $\left(Y_\alpha,\mathcal{N}_\alpha\right)$ be an indexed collection of measurable space.
    The \textbf{product measurable space} is the  $Y= \prod_{\alpha \in A} Y_\alpha$ with \textbf{product $\sigma$-algebra} $\mathcal{N}=\otimes_{\alpha \in A} \mathcal{N}_\alpha$ which is the $\sigma$-algebra generated by $\left\{\pi_\alpha^{-1}\left(E_\alpha\right): E_\alpha \in \mathcal{N}_\alpha, \alpha \in A\right\}$
    \begin{remark}
        The product $\sigma$-algebra is the smallest $\sigma$-algebra on $Y$ such that all the coordinate maps $\pi_\alpha: Y \rightarrow Y_\alpha$ are measurable.
        It follows that $f: X \rightarrow Y$ is $(\mathcal{M}, \mathcal{N})$-measurable iff $\pi_\alpha \circ f$ is $\left(\mathcal{M}, \mathcal{N}_\alpha\right)$-measurable for all $\alpha$.
    \end{remark}
\end{definition}
...
\begin{definition}
    A measure space $\left(X, \mathcal{M}, \mu\right)$ is called \textbf{complete} if every subset of every null set is measurable.
\end{definition}
\begin{proposition}
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item
              ...
              We say $f$ is \textbf{integrable} if both $\int_Ef^+\d \mu$ and $\int_Ef^-\d \mu$ are finite.
    \end{enumerate}
\end{definition}
...
\begin{definition}
    Let $\left(X,\mathcal{M},\mu\right)$ be a measure space and $\left\{f_n\right\}$ be a sequence of measurable complex-valued functions on $\left(X, \mathcal{M}, \mu\right)$.
    \begin{enumerate}
        \item
              We say that $\left\{f_n\right\}$ is \textbf{Cauchy in measure} if for every $\varepsilon>0$,
              \begin{equation*}
                  \mu\left(\left\{x:\left|f_n(x)-f_m(x)\right| \geq \varepsilon\right\}\right) \rightarrow 0 \text { as } m, n \rightarrow \infty,
              \end{equation*}
        \item
              Say that $\left\{f_n\right\}$ \textbf{converges in measure} to $f$ if for every $\varepsilon>0$,
              \begin{equation*}
...
\section{Relation between different convergence modes}
\subsection{$L^p$ and $\mu$}
...
\begin{corollary}[Dominated Convergence Theorem in \texorpdfstring{$L^p$}{}]
    \begin{equation*}
...
Throughout this chapter, $X$ will denote an LCH space.

\section{Basic Definition} % Basic Definition
\begin{definition}
    Let $\mu$ be a Borel measure on locally compact
    Hausdorff space $X$ and $E$ a Borel set of $X$.
    \begin{enumerate}
        \item
              ...
              \end{equation*}
        \item
              The $\mu$ is called \textbf{inner regular} on $E$ if
              \begin{equation*}
                  \mu(E)=\sup\left\{\mu(K):K\subset E,\: K \text{ compact}\right\}
              \end{equation*}
        \item
              The $\mu$ is called \textbf{regular} on $E$ if $\mu$ is both outer and inner regular on $E$.
...
\begin{lemma}
    If $\Lambda$ is a positive linear functional on $C_c(X)$, for each compact $K \subset X$ there is a constant $C_K$ such that $|\Lambda(f)| \leq C_K\|f\|_u$ for all $f \in C_c(X)$ such that $\operatorname{supp}(f) \subset K$.
\end{lemma}
\begin{theorem}[Riesz Representation Theorem]
    ...
    \begin{equation*}
        \mu(U)=\sup \left\{\Lambda f: f \prec U\right\} \text { for every open set } U
    \end{equation*}
    and
    \begin{equation*}
        \mu(K)=\inf \left\{\Lambda f: f \succ K\right\} \text { for every compact set } K
    \end{equation*}
    \begin{proof}
...
\begin{proposition}
    ...
    $C_c(X)$ is dense in $L^p(\mu)$ for $1 \leq p<\infty$.
    \begin{proof}
...
\begin{theorem}[Simple function]
    ...
    $\mathcal{S}=\left\{f=\sum_1^n a_j \chi_{E_j}: \mu\left(E_j\right)<\infty \right\}$  is dense in $L^{p}$.
\end{theorem}
...
\begin{corollary}
    \begin{equation*}
        \int \left|f\right|^p d \mu=-\int_0^{\infty} \alpha^p d \lambda_f(\alpha)
        =
        p \int_0^{\infty} \alpha^{p-1} \lambda_f(\alpha) d \alpha
    \end{equation*}
\end{corollary}
...
\chapter{}
\section{Signed Measures and complex measures}
...
\begin{definition}
    ...
    \end{enumerate}
    \begin{remark}
        It follows that the sum converges absolutely if $\nu\left(\bigcup_1^{\infty} E_j\right)$ is finite.
    \end{remark}
\end{definition}
...
\begin{proposition}
    ...
    \begin{enumerate}
        ...
        \item
              if $\mu$ is a measure on $\mathcal{M}$ and $f: X \rightarrow[-\infty, \infty]$ is a measurable function such that at least one of $\int f^{+} d \mu$ and $\int f^{-} d \mu$ is finite (in which case we shall call $f$ an \textbf{extended $\mu$-integrable} function), then the set function $\nu$ defined by $\nu(E):=\int_E f d \mu$ is a signed measure.
    \end{enumerate}
\end{proposition}
\begin{definition}
    Let $\nu$ be a signed measure on $(X, \mathcal{M})$, a set $E \in \mathcal{M}$ is called \textbf{positive} (resp. \textbf{negative}, \textbf{null}) for $\nu$ if $\nu(F) \geq 0($ resp. $\nu(F) \leq 0, \nu(F)=0)$ for all $F \in \mathcal{M}$ such that $F \subset E$.
\end{definition}

\begin{definition}
    We say that two signed measures $\mu$ and $\nu$ on $(X, \mathcal{M})$ are \textbf{mutually singular}, if there exist $E, F \in \mathcal{M}$ such that $E \cap F=\varnothing, E \cup F=X, E$ is null for $\mu$, and $F$ is null for $\nu$. In this case, we write
    \begin{equation*}
        \mu \perp \nu .
    \end{equation*}
\end{definition}
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item
              $E \in \mathcal{M}$ is $\nu$-null iff $|\nu|(E)=0$,
        \item
              $\nu \perp \mu \Leftrightarrow \left|\nu\right| \perp \mu \Leftrightarrow \nu^{+} \perp \mu$ and $\nu^{-} \perp \mu$

        \item
              If $\nu$ omits the value $+\infty$ then $\nu^{+}(X)=\nu(P)<\infty$, so that $\nu^{+}$is a finite measure and $\nu$ is bounded above by $\nu^{+}(X)$;

              similarly if $\nu$ omits the value $-\infty$.
    \end{enumerate}
\end{proposition}
\begin{definition}
    Let $\nu$ be a signed measure on $(X, \mathcal{M})$.
    The integration of a measurable function $f$ with respect to $\nu$ is defined by
    \begin{equation*}
        \int f \d \nu:=\int f \d \nu^{+}-\int f \d \nu^{-}
    \end{equation*}
    (provided that at least one of the two integrals on the right is finite).
    Thus
    \begin{equation*}
...
\begin{definition}
    A \textbf{complex measure} on a measurable space $(X, \mathcal{M})$ is a map $\nu$ such that
    \begin{enumerate}[label=(\roman*)]
        \item $\nu: \mathcal{M} \rightarrow \mathbb{C}$
        \item
              $\nu(\varnothing)=0$;
        \item
    ...
    \begin{remark}
        ...
        Conversely, if $\nu_r$ and $\nu_i$ are finite signed measures, then $\nu=\nu_r+i \nu_i$ is a complex measure.
    \end{remark}
\end{definition}
...
\section{The Radon-Nikodym Theorem} % Absolute Continuity and Mutual Singularity
\begin{definition}
    Let $\mu$ be a positive measure on $\left(X,\mathcal{M}\right)$, and let $\lambda$ be a signed or complex measure.
    \begin{enumerate}
        \item
              We say that \textbf{$\lambda$ is absolutely continuous with respect to $\mu$}, and write
              \begin{equation*}
                  \lambda \ll \mu
              \end{equation*}
        ...
        \item
              If there is a set $A \in \mathcal{M}$ such that $\lambda(E)=\lambda(A \cap E)$ for every $E \in \mathcal{M}$, we say that \textbf{$\lambda$ is concentrated on $A$}.

        \item
              Suppose $\lambda_1$ and $\lambda_2$ are measures on $\mathcal{M}$, and suppose there exists a pair of disjoint sets $A$ and $B$ such that $\lambda_1$ is concentrated on $A$ and $\lambda_2$ is concentrated on $B$. Then we say that \textbf{$\lambda_1$ and $\lambda_2$ are mutually singular}, and write
              $$
...
\begin{proposition}
    ...
    \begin{enumerate}
        \item
              If $\lambda$ is concentrated on $A$, so is $|\lambda|$.

        \item
              If $\lambda_1 \perp \lambda_2$, then $\left|\lambda_1\right| \perp\left|\lambda_2\right|$.

        \item
              If $\lambda_1 \perp \mu$ and $\lambda_2 \perp \mu$, then $\lambda_1+\lambda_2 \perp \mu$.

        \item
              If $\lambda_1 \ll \mu$ and $\lambda_2 \ll \mu$, then $\lambda_1+\lambda_2<\mu$.

        \item
              $\lambda \ll \mu \Leftrightarrow \left|\lambda\right| \ll \mu $.

        \item
              If $\lambda_1 \ll \mu$ and $\lambda_2 \perp \mu$, then $\lambda_1 \perp \lambda_2$.

        \item
              If $\lambda \ll \mu$ and $\lambda \perp \mu$, then $\lambda=0$.
    \end{enumerate}
\end{proposition}
...
\begin{theorem}
    Let $\mu$ be a positive measure on $\left(X,\mathcal{M}\right)$, and let $\lambda$ be a complex measure on $\mathcal{M}$.
...
\begin{theorem}[The Lebesgue-Radon-Nikodym Theorem]
    Let $\lambda$ be a $\sigma$-finite signed measure and $\mu$ a $\sigma$-finite positive measure on $(X, \mathcal{M})$.
    \begin{enumerate}
        \item
              There exist unique $\sigma$-finite signed measures $\lambda_a, \lambda_s$ on $(X, \mathcal{M})$ such that
              \begin{equation*}
                  \lambda = \lambda_a + \lambda_s, \quad \lambda_a \perp \mu, \quad \lambda_s \ll \mu.
              \end{equation*}
        \item
              There is an unique (in the sense of equality $\mu$-a.e.) extended $\mu$-integrable function $f: X \rightarrow \mathbb{R}$ such that
              \begin{equation*}
                  \d \lambda_s=f \d \mu
              \end{equation*}
    \end{enumerate}
...
\begin{theorem}
    If $\nu$ is a complex measure and $\mu$ is a $\sigma$-finite positive measure on $(X, \mathcal{M})$, there exist a complex measure $\lambda$ and an $f \in L^1(\mu)$ such that $\lambda \perp \mu$ and $d \nu=d \lambda+f d \mu$. If also $\lambda^{\prime} \perp \mu$ and $d \nu=d \lambda^{\prime}+f^{\prime} d \mu$, then $\lambda=\lambda^{\prime}$ and $f=f^{\prime} \mu$-a.e.
\end{theorem}


\section{Consequence Radon-Nikodym Theorem}

\begin{theorem}[Polar Representation of Complex Measure]
    Let $\mu$ be a complex measure on $\left(X,\mathcal{M}\right)$. Then there is a measurable function $h\in L^1 (\left|\mu\right|)$ such that $\left|h(x)\right|=1$ for all $x \in X$ and such that
    \begin{equation*}
        \d \mu
        =
        h \d \left|\mu\right|
...
\begin{theorem}[Lebesgue-Radon-Nikodym]
    Let $\mu$ be a positive $\sigma$-finite measure, and let $\lambda$ be a complex measure on $\mathcal{M}$.
    \begin{enumerate}
        \item
              There is then a unique pair of complex measures $\lambda_{a}$ and $\lambda_s$ on $\left(X,\mathcal{M}\right)$ such that
              \begin{equation*}
        ...
        \item
              ...
              \end{equation*}
              for every set $E \in \mathcal{M}$. We may express this in the form $\d \lambda_{a}=h \d \mu$, or even in the form $h=\d \lambda_{a} / \d \mu$.
    \end{enumerate}

    \begin{remark}
        The pair $\left(\lambda_a, \lambda_b\right)$ is called the \textbf{Lebesgue decomposition of $\lambda$ relative to $\mu$}.
        The function $h$ is called the \textbf{Radon-Nikodym derivative of $\lambda_a$ with respect to $\mu$}.
    \end{remark}
    \begin{proof}
...
\begin{theorem}[Hardy's Inequality]
    ...
    \begin{proof}
        Since $f(t) t^\alpha$ and $t^{-\alpha}$ are nonnegative, we have estimate by Holder's
        $$
            \begin{aligned}
