...
\begin{definition}
    If $\mathscr{F}$ is a presheaf on $X$, and if $P$ is a point of $X$, we define the \textbf{stalk} $\mathscr{F}_P$ of $\mathscr{F}$ at $P$
    \begin{equation*}
        \mathscr{F}_P
        :=
        \lim_{\substack{\longrightarrow \\ P \in U}} \mathscr{F}(U)
    \end{equation*}
    to be the direct limit of the groups $\mathscr{F}(U)$ for all open sets $U$ containing $P$, via the restriction maps $\rho$.
